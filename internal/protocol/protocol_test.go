...
// runs in, else nothing at all. It is never more than the daemon knows, and a
// caller outside a pane is `unknown` rather than anyone.
...
func TestTheWireCarriesOnlyWhatWasSet(t *testing.T) {
	...
	for _, absent := range []string{"project", "all_projects", "pane", "door", "as", "follow", "args"} {
